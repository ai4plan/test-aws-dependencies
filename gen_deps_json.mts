...
function gen_deps_json(
  ...
  function walk(parent: NpmLsNode, parent_path: string) {
    ...
    for (const dep_name of Object.keys(parent.dependencies)) {
      ...
      const dep_path = parent_path ? `${parent_path} > ${dep_name_version}` : dep_name_version

      const dep_entry = getOrCreateInMap(dep_entries, dep_name, () => new Map<string, string[]>())
      const dep_versions = getOrCreateInMap(dep_entry, dep_version, () => [] as string[])
  ...
  writeFile(outputAbsPath, JSON.stringify(outputs, null, 2))
}

function addToGraph(graph: Map<string, Set<string>>, parent: string, child: string): void {
...
async function gen_dep_svg(
  ...
  // dot_content -> file
  writeFile(outputAbsPath + '.dot', JSON.stringify(dot_content, null, 2))

  //---------------------------------------------------------
  // dot_content -> svg_content
  const svg_content = await renderGraphFromSource({ input: dot_content }, { format: 'svg' })
